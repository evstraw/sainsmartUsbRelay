...
static void help(char* myName) {
    ...
    fprintf(stdout, "  --on | -o [1|2|3|4|all]  switch specified relay output on.This argument "
                    "also allows comma seperated relay numbers.\n");
    fprintf(stdout, "  --off | -f [1|2|3|4|all]  switch specified relay output off.This argument "
                    "also allows comma seperated relay numbers.\n");
...
static void checkPermission() {
    if (geteuid() != 0) {
        ...
        fprintf(stderr,
                "Therefore it might not be able to access your relay cards communication port.\n");
        fprintf(stderr,
...
int find_device(void) {
    ...
    char manufacturer[MAX_STR_LEN_MANUFACTURER], description[MAX_STR_LEN_DESCRIPTION],
        serial[MAX_STR_LEN_SERIAL], sainsmartboard[MAX_STR_LEN_SAINSMARTBOARD];
    ...
    for (curdev = devlist; curdev != NULL; curdev = curdev->next) {
        if ((ret = ftdi_usb_get_strings(ftdi, curdev->dev, manufacturer, MAX_STR_LEN_MANUFACTURER,
                                        description, MAX_STR_LEN_DESCRIPTION, serial,
                                        MAX_STR_LEN_SERIAL)) < 0) {
        ...
        if (0 ==
            strncmp(description, SAINSMART_FTDI_CHIP_MODEL, strlen(SAINSMART_FTDI_CHIP_MODEL))) {
            snprintf(sainsmartboard, sizeof(sainsmartboard), "%s", "YES");
            printf(
                "Manufacturer: %-6s, Description: %-20s, Serial: %-10s, Sainsmart Board: %s%s%s\n",
                manufacturer, description, serial, ANSI_COLOR_GREEN, sainsmartboard,
                ANSI_COLOR_RESET);
        } else {
            snprintf(sainsmartboard, sizeof(sainsmartboard), "%s", "NO");
            printf(
                "Manufacturer: %-6s, Description: %-20s, Serial: %-10s, Sainsmart Board: %s%s%s\n",
                manufacturer, description, serial, ANSI_COLOR_RED, sainsmartboard,
                ANSI_COLOR_RESET);
        }
    }
done:
...
int main(int argc, char* argv[]) {
    ...
    static struct option long_options[] = {
        {"help", no_argument, 0, 'h'},         {"findall", no_argument, 0, 'a'},
        {"on", required_argument, 0, 'o'},     {"off", required_argument, 0, 'f'},
        {"status", required_argument, 0, 's'}, {0, 0, 0, 0}};
    if (argc < 2) {
    ...
    while ((opt = getopt_long(argc, argv, ":has:o:f:", long_options, &long_index)) != -1) {
            ...
            case 'o':
                ...
                if (strcasecmp(optarg, "all") == 0) {
                    if (all_check_flag == 1) {
                        fprintf(
                            stderr,
                            "invalid arguments. 'all' value is already set to --off argument\n");
                        exit(EXIT_FAILURE);
                    }
                    all_check_flag = 1;
                    opOn = ID_ON_ALL;
